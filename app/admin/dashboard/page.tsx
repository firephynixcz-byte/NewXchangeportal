...
export default function StaffApprovalPage() {
  ...
  const handleApprove = async (id: string) => {
    // 1. ตรวจสอบข้อมูลผู้ใช้งานก่อนดำเนินการ
    const managerUsername = localStorage.getItem('username');
    if (!managerUsername) {
      alert("ไม่พบข้อมูลผู้ใช้งาน (กรุณา Login ใหม่อีกครั้ง)");
      return;
    }
    ...
    if (res.success) {
      ...
      fetchData(); // รีเฟรชข้อมูลใหม่หลังจากอนุมัติ
    } else {
      ...
      <main className="flex-1 p-4 md:p-10">
        ...
        <div className="bg-white rounded-[2rem] shadow-xl border border-slate-200 overflow-hidden">
          {isLoading ? (
            ...
            <div className="p-20 text-center text-slate-400">ไม่มีพนักงานรออนุมัติในขณะนี้</div>
          ) : (
